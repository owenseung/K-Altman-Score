...
def get_year_end_close(tickers, start="2018-12-01", end="2019-01-10", sleep_sec=0.2):
    ...
    for i, t in enumerate(tickers, 1):
        ...
        try:
            ...
            if hist is None or hist.empty or ("Close" not in hist.columns):
                ...
                target = pd.Timestamp("2018-12-31")
    ...
    return pd.Series(closes, name="Close_2018YE")

def get_year_end_close_safe(tickers, start="2018-12-01", end="2019-01-10", sleep_sec=0.2):
    """
    ...
    for i, t in enumerate(tickers, 1):
        ...
        try:
            ...
            if hist is None or hist.empty or ("Close" not in hist.columns):
                ...
                target = pd.Timestamp("2018-12-31")
                s = h.loc[:target, "Close"].dropna()      # 2018-12-31 이전까지의 마지막 종가
                closes[t] = float(s.iloc[-1]) if len(s) else np.nan
...
# -----------------------------
# 2) FILTER: FY=2018, Form=10-K
# -----------------------------
# Keep only 10-K for FY_TARGET
sub_2018 = sub_all[(sub_all["form"] == "10-K") & (sub_all["fy"] == FY_TARGET)].copy()

# adsh -> (period, cik, name)
adsh_meta = sub_2018[["adsh", "period", "cik", "name"]].drop_duplicates("adsh")
print("2018 10-K filings:", adsh_meta.shape)

# Limit num to only those adsh to reduce work
num_2018 = num_all.merge(adsh_meta[["adsh", "period"]], on="adsh", how="inner")

# Critical: keep only facts at the fiscal-year-end date
num_2018 = num_2018[num_2018["ddate"] == num_2018["period"]].copy()

print("num_2018 (after adsh+ddate==period):", num_2018.shape)
...
stock = num_2018[(num_2018["tag"].isin(STOCK_TAGS)) & (num_2018["qtrs"] == 0)].copy()
...
re_df = num_2018[(num_2018["tag"].isin(RE_CANDIDATES)) & (num_2018["qtrs"] == 0)].copy()
...
flow = num_2018[(num_2018["tag"].isin(FLOW_TAGS + [SHARES_TAG]))].copy()
...
close_map = get_year_end_close_safe(tickers, start="2018-12-01", end="2019-01-10", sleep_sec=0.2)
...
df2["X4"] = np.nan
mask_x4 = df2["Close_2018YE"].notna() & df2["SHARES_DIL_WAVG"].notna() & df2["TL"].notna() & (df2["TL"] != 0)
df2.loc[mask_x4, "X4"] = (df2.loc[mask_x4, "Close_2018YE"] * df2.loc[mask_x4, "SHARES_DIL_WAVG"]) / df2.loc[mask_x4, "TL"]
...
out_cols = [
    ...
    "SHARES_DIL_WAVG", "Close_2018YE",
    "Z_noMVE", "X4", "Z_full",
]
out = df2[out_cols].copy()

out_path = os.path.join(BASE_DIR, "altman_2018_all_companies.csv")
