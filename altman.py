...
BASE_DIR = "/Users/"  # <-fold path
QUARTERS = ["2016q1", "2016q2", "2016q3", "2016q4"]
FY_TARGET = 2015

# EDGAR tags we need
STOCK_TAGS = ["Assets", "AssetsCurrent", "LiabilitiesCurrent", "Liabilities"]  
FLOW_TAGS  = ["OperatingIncomeLoss", "Revenues"]                                

# retained earnings candidates 
RE_CANDIDATES = [
...
def get_year_end_close(tickers, start="2015-12-01", end="2016-01-10", sleep_sec=0.2):
    ...
    for i, t in enumerate(tickers, 1):
        ...
        try:
            ...
            if hist is None or hist.empty or ("Close" not in hist.columns):
                ...
                target = pd.Timestamp("2015-12-31")
    ...
    return pd.Series(closes, name="Close_2015YE")

def get_year_end_close_safe(tickers, start="2015-12-01", end="2016-01-10", sleep_sec=0.2):
    """
    ...
    for i, t in enumerate(tickers, 1):
        ...
        try:
            ...
            if hist is None or hist.empty or ("Close" not in hist.columns):
                ...
                target = pd.Timestamp("2015-12-31")
                s = h.loc[:target, "Close"].dropna()      # 2015-12-31 이전까지의 마지막 종가
                closes[t] = float(s.iloc[-1]) if len(s) else np.nan
...
sub_2015 = sub_all[(sub_all["form"] == "10-K") & (sub_all["fy"] == FY_TARGET)].copy()


adsh_meta = sub_2015[["adsh", "period", "cik", "name"]].drop_duplicates("adsh")
print("2015 10-K filings:", adsh_meta.shape)


num_2015 = num_all.merge(adsh_meta[["adsh", "period"]], on="adsh", how="inner")


num_2015 = num_2015[num_2015["ddate"] == num_2015["period"]].copy()

print("num_2015 (after adsh+ddate==period):", num_2015.shape)


stock = num_2015[(num_2015["tag"].isin(STOCK_TAGS)) & (num_2015["qtrs"] == 0)].copy()
...
re_df = num_2015[(num_2015["tag"].isin(RE_CANDIDATES)) & (num_2015["qtrs"] == 0)].copy()
...
flow = num_2015[(num_2015["tag"].isin(FLOW_TAGS + [SHARES_TAG]))].copy()
...
close_map = get_year_end_close_safe(tickers, start="2015-12-01", end="2016-01-10", sleep_sec=0.2)
...
df2["X4"] = np.nan
mask_x4 = df2["Close_2015YE"].notna() & df2["SHARES_DIL_WAVG"].notna() & df2["TL"].notna() & (df2["TL"] != 0)
df2.loc[mask_x4, "X4"] = (df2.loc[mask_x4, "Close_2015YE"] * df2.loc[mask_x4, "SHARES_DIL_WAVG"]) / df2.loc[mask_x4, "TL"]
...
out_cols = [
    ...
    "SHARES_DIL_WAVG", "Close_2015YE",
    "Z_noMVE", "X4", "Z_full",
]
out = df2[out_cols].copy()

out_path = os.path.join(BASE_DIR, "altman_2015_all_companies.csv")
