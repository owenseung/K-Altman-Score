...
def get_cik_to_ticker_map():
    ...
    rows = []
    # file format: {"0": {"cik_str": 320233, "ticker": "AAPL", ...}, ...}
...
def get_year_end_close(tickers, start="2022-12-01", end="2023-01-10", sleep_sec=0.2):
    ...
    for i, t in enumerate(tickers, 1):
        ...
        try:
            ...
            if hist is None or hist.empty or ("Close" not in hist.columns):
                ...
                target = pd.Timestamp("2022-12-31")
    ...
    return pd.Series(closes, name="Close_2022YE")

def get_year_end_close_safe(tickers, start="2022-12-01", end="2023-01-10", sleep_sec=0.2):
    """
    ...
    for i, t in enumerate(tickers, 1):
        ...
        try:
            ...
            if hist is None or hist.empty or ("Close" not in hist.columns):
                ...
                target = pd.Timestamp("2022-12-31")
                s = h.loc[:target, "Close"].dropna()      # 2022-12-31 이전까지의 마지막 종가
                closes[t] = float(s.iloc[-1]) if len(s) else np.nan
...
# -----------------------------
# 2) FILTER: FY=2022, Form=10-K
# -----------------------------
# Keep only 10-K for FY_TARGET
sub_2022 = sub_all[(sub_all["form"] == "10-K") & (sub_all["fy"] == FY_TARGET)].copy()

# adsh -> (period, cik, name)
adsh_meta = sub_2022[["adsh", "period", "cik", "name"]].drop_duplicates("adsh")
print("2022 10-K filings:", adsh_meta.shape)

# Limit num to only those adsh to reduce work
num_2022 = num_all.merge(adsh_meta[["adsh", "period"]], on="adsh", how="inner")

# Critical: keep only facts at the fiscal-year-end date
num_2022 = num_2022[num_2022["ddate"] == num_2022["period"]].copy()

print("num_2022 (after adsh+ddate==period):", num_2022.shape)
...
stock = num_2022[(num_2022["tag"].isin(STOCK_TAGS)) & (num_2022["qtrs"] == 0)].copy()
...
re_df = num_2022[(num_2022["tag"].isin(RE_CANDIDATES)) & (num_2022["qtrs"] == 0)].copy()
...
flow = num_2022[(num_2022["tag"].isin(FLOW_TAGS + [SHARES_TAG]))].copy()
...
close_map = get_year_end_close_safe(tickers, start="2022-12-01", end="2023-01-10", sleep_sec=0.2)
...
df2["X4"] = np.nan
mask_x4 = df2["Close_2022YE"].notna() & df2["SHARES_DIL_WAVG"].notna() & df2["TL"].notna() & (df2["TL"] != 0)
df2.loc[mask_x4, "X4"] = (df2.loc[mask_x4, "Close_2022YE"] * df2.loc[mask_x4, "SHARES_DIL_WAVG"]) / df2.loc[mask_x4, "TL"]
...
out_cols = [
    ...
    "SHARES_DIL_WAVG", "Close_2022YE",
    "Z_noMVE", "X4", "Z_full",
]
out = df2[out_cols].copy()

out_path = os.path.join(BASE_DIR, "altman_2022_all_companies.csv")
